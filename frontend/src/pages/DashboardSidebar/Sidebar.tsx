...
const Sidebar = ({ className = "", setIsHome }) => {
  ...
  return (
    <div className={`flex flex-col ${className}`}>
      ...
      <Tabs
        value={tab}
        onValueChange={(val) => setTab(val as "rooms" | "search" | "favorites")}
          ...
          {tabs.map((tab) => (
            <TabsTrigger
              ...
              className={` flex flex-col text-xs gap-1 ${
                showSettings || "data-[state=active]:text-[#da7d02] "
              }`}>
              {tab.icon}
              {tab.title}
            </TabsTrigger>
        ...
        {showSettings ? (
          ...
          <>
            <TabsContent value="rooms" className="overflow-y-auto h-full pb-20">
              {rooms.length > 0 ? (
